...
@persistence.persistent_class
class UNetBlock_ctm(torch.nn.Module):
    def __init__(self,
        in_channels, out_channels, emb_channels, up=False, down=False, attention=False,
        num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5,
        resample_filter=[1,1], resample_proj=False, adaptive_scale=True,
...
@persistence.persistent_class
class SongUNet_ctm(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
        ...
        num_blocks          = 4,            # Number of residual blocks per resolution.
        attn_resolutions    = [16],         # List of resolutions with self-attention.
        dropout             = 0.3,         # Dropout probability of intermediate activations.
        label_dropout       = 0,            # Dropout probability of class labels for classifier-free guidance.
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f'{res}x{res}_down'] = UNetBlock_ctm(in_channels=cout, out_channels=cout, down=True,
            ...
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = (res in attn_resolutions)
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock_ctm(in_channels=cin, out_channels=cout, attention=attn,
                                                                training_mode=training_mode, linear_probing=linear_probing, **block_kwargs)
        skips = [block.out_channels for name, block in self.enc.items() if 'aux' not in name]
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = img_resolution >> level
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock_ctm(in_channels=cout, out_channels=cout, attention=True,
                                                         training_mode=training_mode, linear_probing=linear_probing, **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock_ctm(in_channels=cout, out_channels=cout,
                                                         training_mode=training_mode, linear_probing=linear_probing, **block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock_ctm(in_channels=cout, out_channels=cout, up=True,
                                                        training_mode=training_mode, linear_probing=linear_probing, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res in attn_resolutions)
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock_ctm(in_channels=cin, out_channels=cout, attention=attn,
                                                                training_mode=training_mode, linear_probing=linear_probing, **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res}x{res}_aux_up'] = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=0, up=True, resample_filter=resample_filter)
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                x = block(x, emb, emb_s=None if noise_labels_s == None else emb_s,
                          emb_t=emb_t if self.linear_probing else None) if isinstance(block, UNetBlock_ctm) else block(x)
                skips.append(x)
...
@persistence.persistent_class
class SongUNet_ctm_scalar(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
        ...
        self.scalar_head = torch.nn.Sequential(
            torch.nn.Flatten(),
            torch.nn.Linear(model_channels * channel_mult[0] * (img_resolution ** 2), 128),
            torch.nn.SiLU(),
...
@persistence.persistent_class
class DhariwalUNet_ctm_scalar(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
       ...
       model_channels      = 192,          # Base multiplier for the number of channels.
        ...
        for level, mult in enumerate(channel_mult):
            ...
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock_ctm(in_channels=cin, out_channels=cout, attention=(res in attn_resolutions),
                                                                training_mode=training_mode, **block_kwargs)
        skips = [block.out_channels for block in self.enc.values()]
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock_ctm(in_channels=cin, out_channels=cout, attention=(res in attn_resolutions),
                                                                training_mode=training_mode, **block_kwargs)
        self.out_norm = GroupNorm(num_channels=cout)
        self.out_conv = Conv2d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)
    ...
    def forward(self, x, noise_labels, noise_labels_s, class_labels, augment_labels=None):
        ...
        skips = []
        for block in self.enc.values():
            x = block(x, emb, emb_s=None if noise_labels_s == None else emb_s) if isinstance(block, UNetBlock_ctm) else block(x)
            skips.append(x)
...
@persistence.persistent_class
class DhariwalUNet_st(torch.nn.Module):
...
class ECMPrecond(torch.nn.Module):
    ...
    def forward(self, x, dt, sigma, sigma_0, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda') else torch.float32
