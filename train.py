...
@click.option('--precond',       help='Preconditioning & loss function', metavar='fm|sc|ll',             type=click.Choice(['fm', 'sc', 'll']), default='ll', show_default=True)
...
@click.option('--optim',         help='Name of Optimizer', metavar='Optimizer',                     type=str, default='torch.optim.Adam', show_default=True)
...
@click.option('--lr_rampup',     help='Learning rate rampup', metavar='MING',                       type=click.FloatRange(min=0, min_open=False), default=10, show_default=True)
...
@click.option('--double',        help='How often to reduce dt', metavar='TICKS',                    type=click.IntRange(min=1), default=2500000000, show_default=True)
...
@click.option('--tick',          help='How often to print progress', metavar='KIMG',                type=click.FloatRange(min=1), default=60, show_default=True)
...
@click.option('--ckpt',          help='How often to save latest checkpoints', metavar='TICKS',      type=click.IntRange(min=1), default=10, show_default=True)
...
@click.option('--teacher',      help='Teacher model', metavar='PKL|URL',   type=str)
...
@click.option('--wandb_key',          help='wandb login key', metavar='STR',               type=str, default='', show_default=True)
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(P_mean=opts.mean, P_std=opts.std)
    c.optimizer_kwargs = dnnlib.EasyDict(class_name=f'{opts.optim}', lr=opts.lr, betas=[0.9,0.999], eps=1e-8)
    ...
    if opts.arch == 'ddpmpp':
        if opts.precond == 'sc':
            c.network_kwargs.update(model_type='SongUNet_ctm_scalar', embedding_type='positional', encoder_type='standard', decoder_type='standard')
            c.network_kwargs.update(channel_mult_noise=1, resample_filter=[1,1], model_channels=128, channel_mult=[2,2,2])
        elif opts.precond == 'll':
            c.network_kwargs.update(model_type='SongUNet_ctm_scalar', embedding_type='positional', encoder_type='standard', decoder_type='standard')
            c.network_kwargs.update(channel_mult_noise=1, resample_filter=[1,1], model_channels=128, channel_mult=[2,2,2])
        elif opts.precond == 'fm':
        ...
        elif opts.precond == 'sc':
            c.network_kwargs.update(model_type='DhariwalUNet_ctm_scalar', model_channels=192, channel_mult=[1,2,3,4])
        elif opts.precond == 'll':
            c.network_kwargs.update(model_type='DhariwalUNet_ctm_scalar', model_channels=192, channel_mult=[1,2,3,4])

    # Preconditioning & loss function.
    if opts.precond == 'fm':
        c.network_kwargs.class_name = 'training.experimental.networks.FM_Net'
        c.loss_kwargs.class_name = 'training.experimental.loss.FMLoss'
    elif opts.precond == 'sc':
        c.network_kwargs.class_name = 'training.experimental.networks.ShortCut_Net'
        c.loss_kwargs.class_name = 'training.experimental.loss.ShortcutLoss'
    elif opts.precond == 'll':
        c.network_kwargs.class_name = 'training.experimental.networks.ShortCut_Net'
        c.loss_kwargs.class_name = 'training.experimental.loss.LikelihoodLoss'
    else:
    ...
    if opts.wandb and dist.get_rank() == 0:
        ...
        wandb.init(project='imagenet',reinit=True,name=desc, config=c)
